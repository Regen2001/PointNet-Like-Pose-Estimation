...
def main(args):
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        log_string(logger, 'Train Instance Loss: %f' % train_instance_loss)
...
def parse_args():
    ...
    parser.add_argument('--model', default='rotation', help='model name [default: translation]')
    ...
    parser.add_argument('--loss_reduction', type=str, default='sum', help='type of loss function reduction')
...
def train(args, classifier, criterion, trainDataLoader, optimizer, scheduler, epoch, global_epoch, logger):
    ...
    for batch_id, (points, label, target, _, _) in tqdm(enumerate(trainDataLoader, 0), total=len(trainDataLoader), smoothing=0.9):
        optimizer.zero_grad()
