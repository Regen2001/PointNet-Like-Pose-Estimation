...
class Cube_cylinder:
    def __init__(self, size_cylinder=[0.02,0.03], size_cube=[0.05,0.05,0.05], excursion=True, clip=False):
        self.euler = np.asarray([0,0,0])
        self.delta = np.asarray([0,0,0])
        self.excursion = excursion
        cylinder = Cylinder(size_cylinder[0], size_cylinder[1], sample=False)
        cube = Cube(size_cube[0], size_cube[1], size_cube[2], sample=False)

        cylinder.translation([0, 0, size_cylinder[1]/2+size_cube[1]/2])
        if self.excursion:
            cylinder.translation([(size_cube[0]-size_cylinder[0])/4, (size_cube[1]-size_cylinder[0])/3, 0])

        self.points = np.append(cylinder.get_points(), cube.get_points(), axis=0)
    ...
    def path(self, number):
        ...
        self.points_path = 'data/cube_cylinder/cube_cylinder_' + num + '.txt'
        self.translation_path = 'data/cube_cylinder/cube_cylinder_' + num + '_tran.txt'
        self.rotation_path = 'data/cube_cylinder/cube_cylinder_' + num + '_rot.txt'
...
if __name__ == '__main__':
    ...
    points = Cube_cylinder(excursion=True)
