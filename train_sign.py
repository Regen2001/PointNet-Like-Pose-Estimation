...
def parse_args():
    ...
    parser.add_argument('--model', default='sign', help='model name [default: sign]')
...
def train(args, classifier, criterion, trainDataLoader, optimizer, scheduler, epoch, global_epoch, logger):
    ...
    for batch_id, (points, label, _, _, target) in tqdm(enumerate(trainDataLoader, 0), total=len(trainDataLoader), smoothing=0.9):
        optimizer.zero_grad()
        ...
        loss = criterion(pred, target.float().reshape(-1,1))
